...
```
### 2.3 实验总结
实验2的主要目的是将为每个进程维护一个可以在kernel中运行的内存页表，根据内存管理机制，进程在用户态的页内存起始地址都为0，但是对应的实际物理地址却不会都从0开始。因此进程执行过程中需要进行虚实地址的转换，在指针传入内核态以后，需要有一个对应的`kernel-pagetable`。因此实验的目的就是动手构建`kernel-pagetable`在进程进入内核态时使用，实验过程还了解了内存页表的申请和释放，了解进程切换时，页表是如何跟随进程切换。
## 3.Simplify
### 3.1 实验要求
